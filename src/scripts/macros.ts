...
export function namespace(name: string, segment: string) {
  return `globalThis.__kzl_${name} || (globalThis.__kzl_${name}=${segment});`
}
...
export function find(obj: ObjectValue, patternParts: string[]): SearchResults {
  ...
  function traverse(
    ...
    if (Array.isArray(current)) {
      const index = Number.parseInt(patternPart, 10)
...
export async function cases(value: any, logic: any) {
  if (value instanceof Promise)
    value = await value
  const e = () => logic.else?.(value)
  if (value == null)
    return logic.null?.(value) ?? e()
  if (Array.isArray(value))
    return logic.arr?.(value) ?? e()
  if (value instanceof Map)
    return logic.map?.(value) ?? e()
  if (value instanceof Set)
    return logic.set?.(value) ?? e()
  if (typeof value === 'number')
    return logic.num?.(value) ?? e()
  if (typeof value === 'string')
    return logic.str?.(value) ?? e()
  return e()
}
