...
describe('find function', () => {
  ...
  describe('performance and boundary testing', () => {
    ...
    it('should handle deeply nested structures', () => {
      ...
      const pattern = Array.from({ length: 50 }).fill('level').concat(['value']) as string[]
      const result = find(deepObj, pattern)

      const expectedKey = `${Array.from({ length: 50 }).fill('level').join('.')}.value`
